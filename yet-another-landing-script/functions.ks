function setHoverPIDLOOPS {
	...
	SET directionCoefficient TO 180.
}
...
function updateHoverSteering{
	...
	SET steeringDirNonNorm TO steeringDirNonNorm - directionCoefficient.
...
function switchMode { //switches the guidance mode from aerodynamic to propulsive guidance and vice versa
	parameter degreeOffset. //angle in degrees to subtract from steering direction computed by PID. 0 if you want to go forward to the direction to target, 180 if you want to point away from the direction to target.
	SET directionCoefficient to degreeOffset.
}
